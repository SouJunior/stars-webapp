<template>
  <v-form>
    <v-text-field v-model="m.name" label="Nome*" variant="outlined"></v-text-field>
    <v-text-field v-model="m.role" variant="outlined" label="Cargo"></v-text-field>
    <v-row align="center" justify="center">
      <v-col>
        <v-btn color="primary" @click="submit">Salvar</v-btn>
        <v-btn class="ml-3" @click="emit('submitcancel')">Cancelar</v-btn>
      </v-col>
    </v-row>
  </v-form>
</template>
